...
@section('breadcrumb')
    <div class="page-banner-content text-center">
        <h3 class="page-banner-heading text-white pb-15"> {{ __('Training Program') }} </h3>

        <!-- Breadcrumb Start-->
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb justify-content-center">
                <li class="breadcrumb-item font-14"><a href="{{route('instructor.dashboard')}}">{{__('Dashboard')}}</a></li>
                <li class="breadcrumb-item font-14 active" aria-current="page">{{ __('Training Program List') }}</li>
            </ol>
        </nav>
    </div>
@endsection
                        ...
                        <div class="table-responsive table-responsive-xl">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{ __('Upcoming Sessions') }}</th>
                                    <th scope="col">{{ __('Past Sessions') }}</th>
...
{{--                                                <a href="{{ route('live-class.create', [$course->uuid]) }}" class="theme-btn theme-button1 default-hover-btn">{{ __('Create Live Class') }}</a>--}}
                                                <a href="{{ route('program.session', [$course->uuid]) }}" class="theme-btn theme-button1 green-theme-btn default-hover-btn">{{ __('View') }}
                                                    List</a>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <!-- If there is no data Show Empty Design Start -->
                        <div class="empty-data">
                            ...
                            <h5 class="my-3">{{ __('Empty Program Sessions') }}</h5>
                        </div>
