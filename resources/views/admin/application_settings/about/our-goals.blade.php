            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-8">
                    <div class="email-inbox__area  bg-style">
                        ...
                        <form action="{{route('settings.about.our-goals.update')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="custom-form-group mb-3 row">
                                <label for="our_history_title" class="col-lg-3 text-lg-right text-black"> {{ __('Our Goals Title') }} </label>
                                <div class="col-lg-9">
                                    <input type="text" name="our_history_title" id="our_history_title" value="{{ @$aboutUsGeneral->our_history_title }}"
                                           class="form-control" placeholder="{{ __('Type our goals title') }}">
                                </div>
                            </div>
...
{{--                                                </div>--}}
                                                <div class="custom-form-group mb-3 col-lg-9">
                                                    <label for="subtitle_{{ $ourHistory['id'] }}" class="text-lg-right text-black"> {{ __('Goal')}} </label>
                                                    <textarea name="subtitle" class="form-control" rows="1" id="subtitle_{{ $ourHistory['id'] }}" required>{{ $ourHistory->subtitle }}</textarea>
                                                </div>
...
{{--                                            </div>--}}
                                            <div class="custom-form-group mb-3 col-lg-9">
                                                <label for="subtitle" class="text-lg-right text-black"> {{ __('Goal') }} </label>
                                                <textarea name="subtitle" class="form-control" rows="1" id="subtitle" required></textarea>
                                            </div>
