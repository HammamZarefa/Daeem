                        ...
                        <tr>
                            <td class="wishlist-course-item">
                                <div class="card course-item wishlist-item border-0 d-flex align-items-center">
                                    <div class="course-img-wrap flex-shrink-0 overflow-hidden">
                                        ...
                                        @endif
                                        <a href="{{ route('student.my-course.show', @$enrollment->course->slug) }}"><img
                                                src="{{ getImageFile(@$enrollment->course->image_path) }}" alt="course"
                                                class="img-fluid"></a>
                                    </div>
                                    <div class="card-body flex-grow-1">
                                        <h5 class="card-title course-title"><a
                                                href="{{ route('student.my-course.show', @$enrollment->course->slug) }}">{{ @$enrollment->course->title }}</a>
                                        </h5>
                                        <div class="card-text font-medium font-13 mb-1">
                                            <button
                                                class="color-gray2 me-2 font-medium bg-transparent border-0 my-course-give-a-review-btn star-full my-learning-give-review courseReview"
                                                data-course_id="{{ @$enrollment->course->id }}" data-bs-toggle="modal"
                                                data-bs-target="#writeReviewModal"><span class="iconify me-1"
                                                                                         data-icon="bi:star-fill"></span>{{ __('Give Review') }}
                                            </button>
                                            <a target="_blank"
                                               href="{{route('student.download-invoice', [@$enrollment->id])}}"
                                               class="color-gray2 me-2 my-learning-invoice"><img
                                                    src="{{ asset('frontend/assets/img/courses-img/invoice-icon.png') }}"
                                                    alt="report" class="me-1">{{__('Invoice')}}</a>
                                        </div>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="review-progress-bar-wrap">
                                    ...
                                    <div class="barras">
                                        <div class="progress-bar-box">
                                            <div
                                                class="progress-hint-value font-14 color-heading">{{number_format(studentCourseProgress(@$enrollment->course->id, @$enrollment->id), 2)}}
                                                %
                                            </div>
                                            <div class="barra">
                                                <div class="barra-nivel"
                                                     data-nivel="{{number_format(studentCourseProgress(@$enrollment->course->id, @$enrollment->id), 2)}}%"
                                                     style="width: {{number_format(studentCourseProgress(@$enrollment->course->id, @$enrollment->id), 2)}}%;"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="wishlist-add-to-cart-btn">
                                @if(checkIfExpired($enrollment))
                                    <a href="{{ route('student.my-course.show', @$enrollment->course->slug) }}"
                                       class="theme-button theme-button1 theme-button3 font-13">{{ __('View') }}</a>
                                @else
                                    <a href="{{ route('course-details', @$enrollment->course->slug) }}"
                                       class="theme-button theme-button1 theme-button3 font-13">{{ __('Renew') }}</a>
                                @endif
                            </td>
                        </tr>
                    @endif
...
@else
    ...
    <div class="empty-data">
        ...
        <h4 class="my-3">{{ __('Empty Program') }}</h4>
    </div>
