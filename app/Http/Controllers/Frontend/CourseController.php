...
class CourseController extends Controller
{
    ...
    public function allCourses(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            $data['courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->with('course_instructors.user.instructor.instructor_courses')->active()->paginate($this->coursePaginateValue);
        }
        ...
        ->pluck('course_id')
        ->toArray();
        $data['total_courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->active()->count();
        $data['difficulty_levels'] = Difficulty_level::all();
        $data['highest_price'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->max('price');

        $data['random_four_categories'] = Category::active()->inRandomOrder()->limit(4)->get();
        return view('frontend.course.course-list', $data);
    }

    public function allProgrammes (Request $request)
    {
        ...
        if($request->ajax()){
            ...
            $data['lastPage'] = $lastPage;
            $data['html'] = View::make('frontend.program.render-program-list-only', $data)->render();
            $data['status'] = true;
            return response()->json($data);
        }
        else{
        ...
        return view('frontend.program.program-list', $data);
    }

    public function searchCourseList(Request $request)
    {
        if (!get_option('private_mode') || Auth::user()) {
            $data['courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->active()->where('title', 'like', '%' . $request->title . '%')->get();
        } else {
            $data['courses'] = [];
        }
    ...
    public function filterCourseData($request)
    {
        if ($request->category_id) {
            $data['courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->whereCategoryId($request->category_id)->active();
        } elseif ($request->sub_category_id) {
            $data['courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->whereSubcategoryId($request->sub_category_id)->active();
        } else {
            $data['courses'] = Course::where('private_mode', '!=', 1)->where('course_type','<>',COURSE_TYPE_PROGRAM)->active();
        }
