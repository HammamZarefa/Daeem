...
class CartManagementController extends Controller
{
    ...
    public function pay(Request $request)
    {
        ...
        if (is_null($request->payForBecomeCoach)){
            if (is_null($request->payForBecomeOrganization)){
                $order_data = $this->placeOrder($request->payment_method);
            }else{
                $order_data = $this->placeOrderForBecomeOrganization($request->payment_method);
            }
    ...
    private function placeOrderForBecomeOrganization($payment_method)
    {
        DB::beginTransaction();
        try {
            $carts = CartManagement::whereUserId(@Auth::id())
